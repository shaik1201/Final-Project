...
if __name__ == '__main__':
    # Get the scrapped jobs from Indeed

    scrapped_jobs_dict = get_indeed_jobs()
...
# translate_locations_prompt = """
#             You are an AI assistant that translates Hebrew city names to their English equivalents. Given the following list of locations in Hebrew, translate each city name to English and remove any district information. Provide only the city name in English for each entry. If a city name is already in English, leave it unchanged. The output must be directly usable as JSON, meaning it should be a single JSON object with no additional characters or formatting outside of this object. Please ensure the response is precisely in the JSON format, without markdown or other syntactical adornments, to facilitate direct parsing into a database. Moreover, ensure you're using triple quotes around the string that spans multiple lines. Your response should follow this structure: "Hebrew City Name 1". Please ensure the analysis is concise, well-organized, and captures the essence of each section based on the job description provided. The output must be correctly formatted as JSON for easy parsing.
#             Job Location: """